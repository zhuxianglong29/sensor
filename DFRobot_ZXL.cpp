...
DFRobot_ZXL::DFRobot_ZXL(uint8_t mode)
:_mode(*((uint8_t*)&mode))   //知识点：C++中构造函数才有的初始化列表，可以进行变量的初始化，这里的功能是初始化_mode
{

  
}
int DFRobot_ZXL::begin(void)
{
  uint8_t id=0xDF;
  if(readReg(SENSOR_ADDR_ID,&id,1) == 0){//#define SENSOR_ADDR_ID     4 //芯片ID寄存器地址 这里的描述从芯片手册上抄写 ，//readReg用于多类继承
  //readReg的具体功能见下文
...
uint16_t DFRobot_ZXL::soundStrengthDB(void)
{
  ...
  DBG(data.sound);  
  return data.sound << 3; //单位是DB，所以要移位，数据要扩大8倍是真实数据？？
}

/* uint16_t DFRobot_Sensor::soundStrengthDB(void)
{
  uint8_t data;  //data这个结构体变量占一个字节，根据手册上这个寄存器定义的，前4位存放声音强度，后4位存放光线强度
  readReg(SENSOR_ADDR_DATA, &data, sizeof(data));  //从SENSOR_ADDR_DATA（组合数据地址）读数据，存放到data中，所以传的是data的地址
  DBG("sound reg raw data is");
  DBG(data>>4);  
  return (data>>4)<<3; //单位是DB，所以要移位，数据要扩大8倍是真实数据？？
}
 */
 
uint32_t DFRobot_ZXL::lightStrengthLux(void)
{
...
uint8_t DFRobot_ZXL::switchMode(uint8_t mode)
{
  uint8_t tmp;
  #ifdef ENABLE_DBG
...
void DFRobot_ZXL::setLED(uint8_t r, uint8_t g, uint8_t b)
{
  sColor_t data={b>>3,g>>2,r>>3};//
  writeReg(SENSOR_ADDR_LED, &data, sizeof(data));
}

void DFRobot_ZXL::setLED(uint16_t color)
{
  writeReg(SENSOR_ADDR_LED, &color, sizeof(color));
}


DFRobot_ZXL_IIC::DFRobot_ZXL_IIC(TwoWire *pWire, uint8_t mode)
  :DFRobot_ZXL(mode)  //初始化列表：子类用父类给的构造函数（就是上一个框里的函数）从而初始化了父类的私有变量_mode
{
...
int DFRobot_ZXL_IIC::begin(void)
{
  Wire.begin();   //Wire.h（IIC）库函数 初始化wire库
  return DFRobot_ZXL::begin();  //执行上面的初始化函数
}

/* int DFRobot_Sensor_IIC::begin(void)
{
  Wire.begin();   //Wire.h（IIC）库函数 初始化wire库
  int ret = DFRobot_Sensor::begin();  //执行上面的初始化函数
  return ret;
} */



void DFRobot_ZXL_IIC::writeReg(uint8_t reg, void* pBuf, size_t size)//具体过程
{
  if(pBuf == NULL){
          DBG("pBuf ERROR!! : null pointer");//一个判断能增加调试能力，提示输入错误
  }
...
uint8_t DFRobot_ZXL_IIC::readReg(uint8_t reg, void* pBuf, size_t size)
{
  if(pBuf == NULL){
    DBG("pBuf ERROR!! : null pointer");
  }
  uint8_t * _pBuf =  (uint8_t*)pBuf; //传进来的是buf的地址，通过指针修改buf的内容//
...
//SPI部分
DFRobot_ZXL_SPI::DFRobot_ZXL_SPI(SPIClass *pSpi, uint8_t csPin, uint8_t mode)
  :DFRobot_ZXL(mode)
{
  _pSpi = pSpi;
  _csPin = csPin;
...
int DFRobot_ZXL_SPI::begin(void)
{
  pinMode(_csPin, OUTPUT);
  _pSpi->begin();
  return DFRobot_ZXL::begin();
}

void DFRobot_ZXL_SPI::writeReg(uint8_t reg, void* pBuf, size_t size)
{
  if(pBuf == NULL){
  DBG("pBuf ERROR!! : null pointer");
}
...
uint8_t DFRobot_ZXL_SPI::readReg(uint8_t reg, void* pBuf, size_t size)
{
  if(pBuf == NULL){
    DBG("pBuf ERROR!! : null pointer");
  }
