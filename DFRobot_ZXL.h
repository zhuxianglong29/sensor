...
#ifdef ENABLE_DBG
#define DBG(...) {Serial.print("[");Serial.print(__FUNCTION__); Serial.print("(): "); Serial.print(__LINE__); Serial.print(" ] "); Serial.println(__VA_ARGS__);}//可以传参，方便打印调试信息
#else
#define DBG(...)/...宏的可变参
#endif

#define DFRobot_Sensor_IIC_ADDR 0x66 //芯片IIC地址，无变化地址功能
#define DFRobot_Sensor_ID 0xDF //芯片ID
...
#define ERR_IC_VERSION    -2      ///< 芯片版本不匹配
  ...
  typedef struct {
    uint8_t   light: 4;
    uint8_t   sound: 4;
  }__attribute__ ((packed)) sCombinedData_t;//占用一个字节，但是怎么放的？谁在高4位？
    
  typedef struct {
    uint8_t   b: 5;
    uint8_t   g: 6;
    uint8_t   r: 5;
  } __attribute__ ((packed)) sColor_t;//说明是此类的结构体，防止混淆
...
class DFRobot_ZXL_IIC:public DFRobot_ZXL
{
public:
  DFRobot_ZXL_IIC(TwoWire *pWire=&Wire, uint8_t mode=eNomalPrecision+eNormalSpeed+eNormalPower);  //子类的构造函数里加上了默认参数,mode=精度、采样速率、功耗。*pWire=&Wire？
  virtual int begin(void);  //与众不同的功能，在IIC子类中这是初始化IIC
protected:
...
class DFRobot_ZXL_SPI:public DFRobot_ZXL
{
public:
  DFRobot_ZXL_SPI(SPIClass *spi=&SPI, uint8_t csPin=4, uint8_t mode=eNomalPrecision+eNormalSpeed+eNormalPower);  //SPI.h里定义了extern SPIClass SPI;因此取SPI对象的地址就能够指向并使用SPI中的方法  //第二个参数csPin是指定cs接的数字引脚
